...
- **0 账号 / 0 云 / 0 上报** — 全部推理与存储在本地,Rust 内核 + SQLite 日志 + ONNX YOLO11n(CPU 默认,可切 GPU);模型缺位自动回退到启发式检测器。
...
| 层 | 能力 |
|---|---|
| 主屏 / 锁屏 | 主屏全屏遮罩、5 个 React 多窗口、Win32 置顶钩住、浮钟可调置顶与静默 |
| 进程白名单 | 周期扫描运行进程,强制最小化 / 关闭违禁项;白名单配置走设置页搜索添加,存 SQLite `whitelist_json` |
| 休息债务 | 专注时长转换为"债务"(debt),可配下限(默认 180s),Setup 可调,一键结算 |
...
| 系统集成 | 开机自启(plugin-autostart)、系统通知中心 toast(plugin-notification)、紧急热键四选一:双击 ESC / F9 / Ctrl+Shift+E / Ctrl+Alt+Q |
...
现覆盖 84 个单元测试:键盘热键解析、启发式检测器、`is_operating_phone` 边界、周报 PNG 生成、SQLite 日志聚合、路径策略判定、FSM 集成冒烟、L3 原因回看、updater 配置检测、跨周趋势、清空反悔 ROLLBACK 一致性、emergency_hotkey 默认值漂移、L1 进入触发 `NotifyL1`、FSM transition effect(`HideOverlay` / `ShowOverlay` / `StopVision`)6 个 regression test。
...
路径策略手册详见 [附录 B](#user-content-appendix-b)。
...
2. 主界面或 Overlay:点击「注入 L1 / L2 / L3」可无需手机验证 FSM
...
| 反悔 | `clear_all_data_with_snapshot` / `restore_last_snapshot` | 快照写 Rust 侧 Mutex 不经前端 base64 |
...
- **无敏感数据出本机**:摄像头帧只在内存推理,**不落盘**;L3 原因走 SQLite 本地表。
- **无 hardcoded secrets**:`tauri.conf.json` 只有公钥(pubkey),私钥留在 CI / build 机的独立环境。
- **快照保护**:`clear_all_data_with_snapshot` 后的快照字节缓存在 Rust 侧 `AppState::last_clear_snapshot`,`restore_last_snapshot` 时 IPC 直接从 Mutex 取(参考 commit `3bf8f49` F7 修复)。
...
提交约定:feat / fix / docs / chore / test / refactor;中文 commit message,首行 ≤ 72 字。任何涉及 IPC 命令增删请同步 `src/types/tauri-ipc.ts` 与 `src-tauri/src/lib.rs` invoke_handler 注册数组。

---

## 设计

中心词 = **监督**。摄像头为你读帧、滑动窗读持机秒、设置读债务。统一走 zinc 黑灰冷峻族(底 `#161618` / L1 `#52525B` / L2 `#A1A1AA` / L3 `#FAFAFA` / 文字 `#E4E4E7`),三原则:0 渐变 cyan→violet(不贪 Linear / Vercel 默认气质)、0 强彩告警三色(L1–L3 靠灰度阶进传"紧",不是荧光)、参照真实业界(Cold Turkey 工具感、RescueTime 严肃、Forest 造物生态)。品牌过程收敛于 hero 顶部三圈 iris(L1 暗→L3 亮,SteelTeal 镜心)。logo 源文件与导出资产存 `logos/`(concepts 3 + iterations 18 + export 10 个尺寸 PNG + `icon.ico`)。
...
**副作用**:紧急退出会**立即结束当前专注会话**,该次会话不计入今日专注;若处于 L3 颂罚,会写入 `EMERGENCY_EXIT` 日志事件(`focus_logs` 表,`event_type='emergency_exit'`),并在周报中以「中断」统计。同时隐藏浮钟、复位遮罩契约。
...
1. **构建** — `npm run tauri build` 成功,产物含 `DeepFlow_x.y.z_x64-setup.exe`
2. **干净机 / 干净用户安装** — 安装后启动,设置页「数据目录」=`%LOCALAPPDATA%\DeepFlow\data`,`get_path_info.mode` = `install`
3. **写入验证** — 完成一次专注会话后 `%LOCALAPPDATA%\DeepFlow\data\deepflow.db` 存在且增长;周报 PNG 落在 `exports/`
4. **便携对照** — 把安装产物旁放 `portable.flag` 再启动,mode 切到 `portable`,数据写到 exe 旁 `data/`
5. **环境变量覆盖** — 设置 `DEEPFLOW_DATA_DIR=D:\\tmp\\df` 启动,mode=`env`,写入该目录
6. **系统集成** — 「登录自启」后任务管理器「启动」可见 DeepFlow;「测试通知」弹出系统中心 toast;「检查更新」未配置 endpoint / pubkey 时返回 `UpdaterConfigStatus::NotConfigured`
